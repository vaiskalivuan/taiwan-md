...
**適用**：文章已存在，但品質不夠（虎頭蛇尾、清單堆砌、後半段崩塌等）。

**核心原則**：疊加在前人成果上，不推倒重來。

**Stage 0: 診斷現有文章（進化模式專屬）**

在 Stage 1 之前，先讀完整篇現有文章，做以下診斷：

1. **標記保留段**：哪些段落已經寫得好？（有場景、有數字、有引語）→ 標記 `[KEEP]`
2. **標記病灶段**：哪些段落有問題？→ 標記問題類型：
   - `[LIST-DUMP]` — 清單堆砌，沒有敘事（最常見的虎頭蛇尾病因）
   - `[THIN]` — 太薄，缺少具體事實/場景
   - `[STALE]` — 資料過時
   - `[PLASTIC]` — 塑膠句/空洞修飾
   - `[FLAT-END]` — 結尾罐頭/無餘韻
3. **列出進化目標**：「這次進化要解決哪些病灶？」
4. **提取現有素材**：從 `[KEEP]` 段落中提取可重複使用的事實、數字、引語

**然後進入 Stage 1（研究），但研究範圍縮小到病灶段需要的素材。**

### 全新模式（Fresh）

**適用**：文章不存在，或品質差到保留成本 > 重寫成本。
